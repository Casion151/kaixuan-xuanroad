...
## 给tutorial编写者：一条推荐写到什么程度才够？

可以用读者点开链接前后的行为判断。点开前，他应已知道当前问题、术语含义，以及为什么这个资源适合回答它；点开后，应能找到具体小节，知道何时停止，最后用一道预测题、一个解释或一个小结果检查理解。

每次更新，先完成一个学习单元：问题引入→必要概念的中文解释→小节/图号导航→第一次任务→参考检查点→卡住时的回补位置。课程简介、难度和时长放在旁边帮助选择，不承担全部教学解释。论文也采用同样方式，不能仅在题名后写“学习PCA”或“理解NeuroAI”。

本版于2026-09-08对照官方课程小节与三篇论文的实际图注/相关正文编写。教学例子与解释为本稿组织；时间为估计。首轮任务可通过阅读和纸笔完成；本次没有运行外部notebook或下载课程数据集。书目与资源许可核查记录、全文定位证据在`evidence/`；完整引文在`references.bib`。
